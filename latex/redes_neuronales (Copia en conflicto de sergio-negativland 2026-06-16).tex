...
La red profunda m\'as simple: capas de neuronas donde cada una recibe de la capa inferior e influye en la superior. Con suficientes neuronas y datos, aprende relaciones que un modelo lineal no puede representar.\footnote{Contenido basado en Zhang, Lipton, Li \& Smola, \emph{Dive into Deep Learning}, cap.~5. \url{https://d2l.ai}}
\end{block}
\end{frame}
...
%=====================================================================
\section{Caso pr\'actico y resumen}
%=====================================================================

\begin{frame}
\frametitle{Caso pr\'actico: precios de viviendas (Kaggle)}
Un flujo t\'ipico para aplicar un MLP a datos tabulares reales:
\begin{enumerate}
	\item \textbf{Preprocesamiento}: estandarizar variables num\'ericas (media $0$, varianza $1$) y codificar las categ\'oricas (\emph{one-hot}).
	\item \textbf{M\'etrica}: para precios conviene el \emph{error cuadr\'atico medio en escala logar\'itmica} (RMSE log), pues importa el error \emph{relativo}.
	\formula{\[
	\sqrt{\frac{1}{n}\sum_{i=1}^{n}\big(\log \hat{y}_i - \log y_i\big)^2}
	\]}
	\item \textbf{Validaci\'on cruzada $K$-fold}: estimar el error de generalizaci\'on y ajustar hiperpar\'ametros (capas, ancho, $\lambda$, \emph{learning rate}).
	\item \textbf{Entrenar} con todos los datos y generar predicciones.
\end{enumerate}
\end{frame}
